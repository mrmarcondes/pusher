  ...
  '<div class="pusher-chat-widget">' +
    ...
    '<div class="pusher-chat-widget-messages">' +
      '<ul class="activity-stream">' +
        '<li class="waiting">Sem mensagens</li>' +
      '</ul>' +
    '</div>' +
    '<div class="pusher-chat-widget-input">' +
      '<label for="message">Mensagem</label><br>' +
      '<textarea name="message"></textarea>' +
      '<button class="pusher-chat-widget-send-btn">Enviar</button>' +
    '</div>' +
  '</div>';
  var widget = $(html);
...
PusherChatWidget.timeToDescription = function(time) {
  ...
  if(seconds === 0) {
    ...
    desc = seconds + " segundo" + (seconds !== 1?"s":"") + " atrás";
  }
  else if(minutes < 60) {
    desc = "about " + minutes + " minuto" + (minutes !== 1?"s":"") + " atrás";
  }
  else if(hours < 24) {
    desc = "about " + hours + " hora"  + (hours !== 1?"s":"") + " atrás";
  }
  else {
    desc = time.getDay() + " " + ["Jan", "Fev", "Mar", "Abr", "Mai", "Jun", "Jul", "Ago", "Set", "Out", "Nov", "Dez"][time.getMonth()]
  }
  return desc;
};
